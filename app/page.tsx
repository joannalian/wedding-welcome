...
function CakeMetric({title,stock,planned,delivered,tone}:{title:string;stock:number;planned:number;delivered:number;tone:string}) { return <article className={`cake-metric ${tone}`}><h3>{title}</h3><div><span><small>庫存數量</small><strong>{stock}<i>盒</i></strong></span><span><small>應發數量</small><strong>{planned}<i>盒</i></strong></span><span><small>已發數量</small><strong>{delivered}<i>盒</i></strong></span></div></article>; }
